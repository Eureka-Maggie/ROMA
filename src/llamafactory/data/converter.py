...
from .data_utils import Role, StreamingRole
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        messages = example[self.dataset_attr.messages] #self.dataset_attr.messages = "messages"
        if (
            self.dataset_attr.system_tag #None
            and len(messages) != 0
        ...
        aligned_messages = []
        broken_data = False 
        for turn_idx, message in enumerate(messages): # message是一个content一个role的那种 {'content': '<video><audio>What is the video describing?', 'role': 'user'}
            if message[self.dataset_attr.role_tag] not in accept_tags[turn_idx % 2]: #accept_tags (('user', None), ('assistant', None))
            ...
            aligned_messages.append(
                {
                    "role": tag_mapping[message[self.dataset_attr.role_tag]], # 这个tag_mapping的逻辑是：user就是user，assistant就是assistant，none就system
...
def get_dataset_converter(name: str, dataset_attr: "DatasetAttr", data_args: "DataArguments") -> "DatasetConverter":
    ...
    return DATASET_CONVERTERS[name](dataset_attr, data_args) #[alpaca](identify.json, DataArguments)
...
def align_dataset(
    ...
    _audios: []
    """
    column_names = list(next(iter(dataset)).keys()) #['task', 'id', 'videos', 'images', 'ans', 'query']
    kwargs = {}
    ...
    return dataset.map(  #调用 DatasetConverter.__call__
        dataset_converter,
        batched=False,
