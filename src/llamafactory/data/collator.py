...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        batch_images, batch_videos, batch_audios, messages = [], [], [], []
        batch_imglens, batch_vidlens, batch_audlens, batch_input_ids = [], [], [], []

        anchor_lists, gate_lists = [], []

        for feature in features:
            anchor_lists.append(feature.pop("anchor_idx_list", []))
            gate_lists.append(feature.pop("gate_label_list", []))

            images = feature.pop("images", None) or []
            videos = feature.pop("videos", None) or []
            audios = [] #[1] if (151647 in feature['input_ids']) else 0 #feature.pop("audios", None) or []
            batch_images.extend(images)
            #print("[DEBUG] feature.videos len:", len(videos))
            batch_videos.extend(videos)
            batch_audios.extend(audios)
            batch_imglens.append(len(images))
            batch_vidlens.append(len(videos))
            #batch_audlens.append(1 if (151647 in feature['input_ids']) else )
            if 151647 in feature['input_ids']:
                batch_audlens.append(1)
            batch_input_ids.append(feature["input_ids"])
            feature.pop("audios", None) 
            try:
                messages.extend(feature.pop('messages'))
            except:
                print("有问题的数据是：",videos,feature['input_ids'])
        ...
        if ( #一个batch都没有image
            ...
            fake_messages = [[{"text":IMAGE_PLACEHOLDER,"audio":fake_audios,"time":0.0}],[{"text":"\n","time":0.0}]] #[{"role": "user", "content": IMAGE_PLACEHOLDER}]
        ...
        if (
            ...
            fake_audios = [np.zeros(1600)]
            fake_messages = [[{"text":"\n","audio":fake_audios,"time":0.0}],[{"text":"\n","time":0.0}]] #[{"role": "user", "content": AUDIO_PLACEHOLDER}]
            # fake_messages = self.template.mm_plugin.process_messages(
            #     fake_messages, [], [], fake_audios, self.processor
            # )
            fake_messages = self.template.mm_plugin.process_messages(
                fake_messages, [], [], [], self.processor
            )
            _fake_input_ids = self.tokenizer.encode(fake_messages[0]["content"], add_special_tokens=False) #self.tokenizer.encode(fake_messages[0]["content"], add_special_tokens=False)
            _fake_input_ids, _ = self.template.mm_plugin.process_token_ids(
        ...
        # 总而言之这一段就是给补了一个audio，video 还是那个 video。但是如果给了这个audio，在mm_plugin里处理的时候，就会用这个audio而不用messages里的query的audio了。所以不能给
        ...
        B = len(anchor_lists)
        M = max((len(x) for x in anchor_lists), default=0)

        if M == 0:
            # 没有任何可判别边界，给一个占位（mask 全 False）
            features["anchor_idx"] = torch.zeros((B, 1), dtype=torch.long)
            features["gate_label"] = torch.zeros((B, 1), dtype=torch.float32)
            features["gate_mask"]  = torch.zeros((B, 1), dtype=torch.bool)
        else:
            anchor_idx = torch.zeros((B, M), dtype=torch.long)
            gate_label = torch.zeros((B, M), dtype=torch.float32)
            gate_mask  = torch.zeros((B, M), dtype=torch.bool)
            for i, (ai, gi) in enumerate(zip(anchor_lists, gate_lists)):
                m = len(ai)
                if m > 0:
                    anchor_idx[i, :m] = torch.tensor(ai, dtype=torch.long)
                    gate_label[i, :m] = torch.tensor(gi, dtype=torch.float32)
                    gate_mask[i, :m]  = True
            features["anchor_idx"] = anchor_idx
            features["gate_label"] = gate_label
            features["gate_mask"]  = gate_mask
        ...
        if self.model is not None and hasattr(self.model, "get_rope_index"):  # for qwen2vl mrope
            ...
            if "second_per_grid_ts" in mm_inputs:  # for qwen2vl #nope 
                rope_index_kwargs["second_per_grid_ts"] = mm_inputs.get("second_per_grid_ts")
            if "video_second_per_grid" in mm_inputs:  # for qwen2omni
                rope_index_kwargs["second_per_grids"] = mm_inputs.get("video_second_per_grid") # tensor([1.0428])

            if getattr(self.model.config, "model_type", None) == "qwen2_5_omni_thinker":  # for qwen2omni
                rope_index_kwargs["use_audio_in_video"] = getattr(self.processor, "use_audio_in_video", False) #True
                feature_attention_mask = mm_inputs.get("feature_attention_mask", None) #tensor([[1, 1, 1,  ..., 0, 0, 0]], dtype=torch.int32)
                if feature_attention_mask is not None:
                    audio_feature_lengths = torch.sum(
                        feature_attention_mask, dim=1
                    )  # FIXME need to get video image lengths #[800,8200]
                    rope_index_kwargs["audio_seqlens"] = audio_feature_lengths  # prepare for input

                delta0 = (1 - rope_index_kwargs["attention_mask"]).sum(dim=-1).unsqueeze(1)
                # avoid conflict
                new_position_ids, rope_deltas = self.model.get_interleaved_rope_index(**rope_index_kwargs) #self.model.get_rope_index(**rope_index_kwargs)
