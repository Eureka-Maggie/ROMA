...
class Template:
    ...
    def _convert_elements_to_ids(self, tokenizer: "PreTrainedTokenizer", elements: "SLOTS") -> list[int]:
        ...
        return token_ids
    def clean_lists(self,data):
        """
        data: list[list[int]]
        return: processed list[list[int]]
        """
        tail_pattern = [151645, 198, 151644, 77091, 198]
        middle_pattern = [27, 91, 34804, 763, 91, 29, 151645, 198] #[151665, 151645, 198] #
        head_pattern = [151644, 872, 198]

        i = 0
        while i < len(data) - 2:
            first, second, third = data[i], data[i+1], data[i+2]

            if first[-len(tail_pattern):] == tail_pattern \
            and second == middle_pattern \
            and third[:len(head_pattern)] == head_pattern:

                # 1. 去掉第一个 list 的尾部
                data[i] = first[:-len(tail_pattern)]
                # 2. 第二个 list 清空
                data[i+1] = []
                # 3. 去掉第三个 list 的头部
                data[i+2] = third[len(head_pattern):]
    ...
    def _encode(
        ...
        for i, message in enumerate(messages):
            ...
            if message["role"] == Role.USER:
                elements += self.format_query.apply(content=message["content"], idx=str(i // 2)) #self.format_user.apply(content=message["content"], idx=str(i // 2))
            elif message["role"] == Role.ASSISTANT:
                elements += self.format_answer.apply(content=message["content"]) #self.format_assistant.apply(content=message["content"])
        ...
        encoded_messages_multi_turn = self.clean_lists(encoded_messages) #encoded_messages 
        return encoded_messages_multi_turn
...
def register_template(
    ...
    format_query: Optional["Formatter"] = None,
    format_answer: Optional["Formatter"] = None,
    ...
    default_query_formatter = StringFormatter(slots=["{{content}}"])
    default_answer_formatter = StringFormatter(slots=["{{content}}"])
    ...
    TEMPLATES[name] = template_class(
        ...
        format_query= format_query or default_query_formatter,
        format_answer=format_answer or default_answer_formatter,
...
register_template(
    name="streaming_mix",
    format_query=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_answer=StringFormatter(slots=["{{content}}<|im_end|>\n"]),
    format_system=StringFormatter(slots=["<|im_start|>system\n{{content}}<|im_end|>\n"]),
    default_system=
    '''You are an assistant that receives streaming multimodal inputs. You must continuously monitor the synchronized video and audio each second and respond at the appropriate time. Provide answers to the audio query based on the visual context at the right moment.
    [narration time] 1. You should only predict a response when an event transition occurs, and at that moment summarize the previous event. After responding, you must not predict another response until the next event transition.
    [narration content] 2. When a response is required, follow the instructions and give a concise summary of the most recent event that just occurred.
    [proactive alert] 1. Only when the condition described in the user's audio instruction is actually satisfied in the current video/audio input are you allowed to speak; otherwise you must output exactly "no".
    [proactive alert] 2. When the condition is satisfied, if the instruction specifies an explicit response you must output that exact content, otherwise you must output exactly "alert".
    ''',
    stop_words=["<|im_end|>"],
    replace_eos=True,
    mm_plugin=get_mm_plugin(
