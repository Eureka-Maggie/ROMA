...
def merge_lora(
    ...
    model.save_pretrained(save_path, safe_serialization=True, max_shard_size="10GB")
...
# ------------------------------
def _ensure_gate_exists(thinker: Qwen2_5OmniThinkerForConditionalGeneration):
    """
    若类定义默认不带 gate_*，先补挂空模块，便于后续从分片回填权重。

    现在不仅要保证 gate_head / gate_mixer 存在，也要保证新的 MLP 头：
    - gate_head_pro_fc1
    - gate_head_pro_act
    - gate_head_pro_fc2
    存在，这样从分片里 load_state_dict 时不会因为没有属性而报错。
    """
    # 如果已经有这些属性就不动（新版 modeling 里应该都有）
    has_head = hasattr(thinker, "gate_head")
    has_mixer = hasattr(thinker, "gate_mixer")
    has_pro_fc1 = hasattr(thinker, "gate_head_pro_fc1")
    has_pro_act = hasattr(thinker, "gate_head_pro_act")
    has_pro_fc2 = hasattr(thinker, "gate_head_pro_fc2")

    # 如果都在，就直接返回
    if has_head and has_mixer and has_pro_fc1 and has_pro_act and has_pro_fc2:
        return

    # 兜底挂默认模块
    text_cfg = thinker.config.get_text_config()
    H = text_cfg.hidden_size
    gate_hidden_dim = getattr(thinker.config, "gate_hidden_dim", H // 4)
    ...
    if not has_pro_fc1:
        thinker.gate_head_pro_fc1 = torch.nn.Linear(H, gate_hidden_dim, bias=True).to(
            device=device, dtype=dtype
        )
    if not has_pro_act:
        thinker.gate_head_pro_act = torch.nn.GELU()
    if not has_pro_fc2:
        thinker.gate_head_pro_fc2 = torch.nn.Linear(gate_hidden_dim, 1, bias=True).to(
            device=device, dtype=dtype
        )


def _try_load_gate_from_shards(thinker, path: str):
    """
    ...
    - thinker.gate_head.*
    - thinker.gate_mixer.*
    - thinker.gate_head_pro_fc1.*
    - thinker.gate_head_pro_fc2.*
    """
    try:
    ...
    for fp in files:
        with safe_open(fp, framework="pt", device="cpu") as f:
            for k in f.keys():
                ...
                elif k.startswith("gate_head_pro_fc1."):
                    pro1_sd[k.split("gate_head_pro_fc1.", 1)[1]] = f.get_tensor(k)
                elif k.startswith("gate_head_pro_fc2."):
                    pro2_sd[k.split("gate_head_pro_fc2.", 1)[1]] = f.get_tensor(k)
                ...
                elif k.startswith("thinker.gate_head_pro_fc1."):
                    pro1_sd[k.split("thinker.gate_head_pro_fc1.", 1)[1]] = f.get_tensor(k)
                elif k.startswith("thinker.gate_head_pro_fc2."):
                    pro2_sd[k.split("thinker.gate_head_pro_fc2.", 1)[1]] = f.get_tensor(k)
    ...
    if head_sd:
        ...
        loaded_any = True
        print("load head_sd done")
    if mixer_sd:
        thinker.gate_mixer.load_state_dict(mixer_sd, strict=False)
        loaded_any = True
        print("load mixer_sd done")
    if pro1_sd:
        thinker.gate_head_pro_fc1.load_state_dict(pro1_sd, strict=False)
        loaded_any = True
        print("load pro1_sd done")
    if pro2_sd:
        thinker.gate_head_pro_fc2.load_state_dict(pro2_sd, strict=False)
        loaded_any = True
        print("load pro2_sd done")

    if loaded_any:
        print("✅ 从 safetensors 回填 gate_head / gate_mixer / gate_head_pro_fc1 / gate_head_pro_fc2 完成")
    else:
